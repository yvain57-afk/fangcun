...
actor ReadinessDemoProvider: ReadinessDataProviding {
  ...
  func changes(for metric: ReadinessMetric, cursor: HealthReadCursor?, now: Date) async throws -> ReadinessChangeBatch {
    ...
    let days = scenario == "provisional" ? 8 : 20
    if scenario != "insufficient", cursor == nil {
      for day in 0...days {
        ...
        rows.append(.init(id: id(1), metric: .sleep, unit: "category", stage: .asleep,
          start: end.addingTimeInterval(-8*3600), end: end, source: source, queriedAt: now))
        for n in 0..<4 {
          let date = end.addingTimeInterval(-Double(n+1)*3600)
          rows.append(.init(id: id(10+n), metric: .hrvSDNN, value: 50, unit: "ms", start: date, end: date, source: source, queriedAt: now))
        }
        if scenario != "limited" {
    ...
    return .init(metric: metric, samples: rows.filter { $0.metric == metric },
