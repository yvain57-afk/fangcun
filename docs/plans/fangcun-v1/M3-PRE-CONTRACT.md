...
基线 b80f81c，工作区干净；Xcode 27 beta / Swift 6，既有 iOS 18 与 watchOS 11 targets 不变。工程运行于 Code/fangcun；测试/构建/结果包位于 Code scratch。当前有一个连接的 iPhone，Watch 连接不可用；真实查询和签名后续单独核验。
